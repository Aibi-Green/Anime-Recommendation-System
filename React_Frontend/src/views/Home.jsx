import React from 'react';
import Marquee from 'react-fast-marquee';
import { Fade, Slide } from 'react-awesome-reveal';
import { LoginPages, MainPages } from '../routes/paths';

import sprite from '/sprite.svg';
import landing from '../assets/home/landingbg.png';
import landingBack from '../assets/home/landingback.png';
import landing1 from '../assets/home/salad.png';
import landing2 from '../assets/home/fruitsandvegetables.png';
import mission from '../assets/home/mission.png';
import vision from '../assets/home/vision.png';
import testimonial from '../assets/home/testimonialbg.png';
import juan from '../assets/testimonial/juan.png';
import marites from '../assets/testimonial/marites.png';
import diluc from '../assets/testimonial/diluc.png';

import Button from '../components/Button';
import MVCard from '../components/MVCard';
import CategoryCard from '../components/CategoryCard';
import TestimonialCard from '../components/TestimonialCard';

const categories = [
  {
    title: <>Educational<br/>Resources</>,
    desc: "Empower your mind with our comprehensive educational resources. Explore, learn, and thrive.",
    bg: "/category/educ-resource.png", link: MainPages.RESOURCES
  },
  {
    title: "Self-Assessment",
    desc: "Take charge of your mental well-being. Start your journey with our self-assessment tools.",
    bg: "/category/self-assess.png", link: MainPages.ASSESSMENT
  },
  {
    title: <>Support<br/>Community</>,
    desc: "Find strength in community. Join our supportive network and journey together towards mental wellness.",
    bg: "/category/support.png", link: MainPages.COMMUNITIES
  },
  {
    title: "Expert's Advice",
    desc: "Take the first step towards mental wellness. Schedule a confidential consultation with our compassionate experts today.",
    bg: "/category/advice.png", link: MainPages.CONSULTATION
  },
  {
    title: "Testimonials",
    desc: "Discover tailored solutions for your mental well-being. Get personalized recommendations from our team of experts.",
    bg: "/category/testimonials.png", link: "#testimonials"
  },
  {
    title: <>Privacy<br/>and Security</>,
    desc: "Your privacy and security are our top priorities. Rest assured, your information is kept confidential and secure with us.",
    bg: "/category/privacy.png", link: MainPages.PRIVACY
  },
  {
    title: <>Health<br/> News and Updates</>,
    desc: "Stay informed, stay empowered. Explore the latest health news and updates to support your well-being journey.",
    bg: "/category/updates.png", link: MainPages.NEWS
  },
  {
    title: <>Emergency<br/>Hotline</>,
    desc: "Empower your mind with our comprehensive educational resources. Explore, learn, and thrive.",
    bg: "/category/hotline.png", link: MainPages.HOTLINES
  },
];

const testimonials = [
  {
    name: "Juan Dela Cruz", image: juan,
    review: "NutriMatch has completely changed the way I approach my diet and overall health. The personalized meal plans and nutritional advice were spot on, aligning perfectly with my goals to improve energy levels and manage weight."
  },
  {
    name: "Marites", image: marites,
    review: "NutriMatch has been a game-changer for me! I've struggled with finding the right balance in my diet, but their personalized nutrition plans have been incredibly helpful. I've noticed a significant improvement in my energy levels and overall health since starting with NutriMatch."
  },
  {
    name: "Diluc Ragnvindr", image: diluc,
    review: "I can't believe how much NutriMatch has simplified healthy eating for me. The meal plans are practical, delicious, and perfectly aligned with my fitness goals. NutriMatch has truly exceeded my expectations!"
  },
];

const Home = () => {
  return (
    <>
    {/* Landing */}
    <div className='relative flex items-center gap-16 w-full h-[95vh] pl-16 -mt-[3vh]'>
      {/* Logo */}
      <div className='group relative grid items-center justify-items-center select-none'>
        <img src='/logo_mini.png' className='col-start-1 row-start-1 w-40 h-auto py-7 bg-secondary-100 rounded-full' />
        <div className='col-start-1 row-start-1 w-24 h-24 py-7 bg-tertiary-400 rounded-full -z-10'></div>
        <svg className='absolute col-start-1 row-start-1 ml-20 w-[30rem] h-[30rem] -z-20 fill-current'>
          <use xlinkHref={`${sprite}#splash`} />
        </svg>
      </div>

      {/* Tagline and description */}
      <Fade direction='up' className='w-[40%]'>
      <div className='flex flex-col items-start gap-5 w-full'>
        <h1 className='text-secondary-200 text-5xl font-bold'>Eat Smart<br/>Live Healthy</h1>
        <p className='text-secondary-100 text-lg'>“NutriMatch is a personalized nutrition service designed to optimize your health. <br/>By analyzing your unique dietary needs and preferences, NutriMatch creates tailored meal plans and nutritional advice to help you achieve your wellness goals. <br/>Whether you want to lose weight, gain muscle, or simply eat healthier, NutriMatch provides customized solutions that fit your lifestyle.”</p>
        <Button link={LoginPages.LOGIN}>Get Started</Button>
      </div>
      </Fade>

      {/* Design in right */}
      <Slide direction='right' className='absolute right-0 top-0 w-full h-full -z-10 select-none' cascade>
      <img src={landingBack} className='absolute right-0 top-0 w-[53%] h-full -z-20 ' />
      <img src={landing} className='absolute right-0 top-0 w-1/2 h-full -z-10 ' />
      </Slide>
      <img src={landing1} className='absolute right-0 top-0 w-[32vw] h-auto select-none' />
      <img src={landing2} className='absolute right-0 bottom-0 w-[43.5vw] h-[47.5vh] select-none' />
    </div>

    {/* About */}
    <div className='grid grid-rows-1 grid-cols-2 w-full h-screen bg-secondary-200 text-primary'>
      <div className='bg-[url("/src/assets/home/nutritionfacts.jpeg")] bg-cover bg-center'></div>
      <Slide direction='right' className='flex overflow-hidden'>
      <div className='flex flex-col items-end gap-5 px-16 my-auto text-right overflow-hidden'>
        <h2 className='text-sm'>About Us</h2>
        <h1 className='text-4xl font-bold'>WHO ARE WE</h1>
        <p>
        At NutriMatch, we are committed to empowering individuals to achieve optimal health and wellness through personalized nutrition solutions. Our dedicated team harnesses the power of personalized data to create tailored meal plans and guidance that fit your unique needs and goals.

        Through our innovative platform and personalized approach, NutriMatch aims to revolutionize how people approach nutrition. We believe that everyone deserves access to customized dietary advice that considers their individual health profile, preferences, and lifestyle.

        With a focus on science-backed nutrition and practical advice, NutriMatch strives to make healthy eating simple and sustainable. Whether you're looking to manage weight, improve athletic performance, or enhance overall well-being, we're here to support you every step of the way.
        </p>
        <Button theme='light' link={MainPages.ABOUT}>Learn More</Button>
      </div>
      </Slide>
    </div>

    {/* Mission Vision */}
    <div className='flex flex-col justify-center gap-16 p-32 w-full h-screen'>
      <Fade direction='left' duration={1750}>
      <MVCard image={mission} title='Mission'>
        Our goal at NutriMatch is to provide individualized nutrition solutions to enable people, to adopt healthier lifestyles. Our dedication lies in offering easily accessible, scientifically supported dietary recommendations that promote overall wellbeing, and boost energy levels. We hope to give people the information and resources they need to make educated food decisions that will improve their personal life by utilizing individualized data and professional insights.
      </MVCard>
      </Fade>
      <Fade direction='right' duration={1750}>
      <MVCard image={vision} title='Vision'>
        Our goal at NutriMatch is to see individualized nutrition recognized as a fundamental component of each person's health and wellbeing. We see a society in which each individual has access to individualized nutrition plans that meet their specific requirements and dietary choices. Our dedication to individualized nutrition is intended to foster a culture in which people are confident in their ability to make dietary decisions that will best support their physical and emotional well-being. We want to build a society where everyone may prosper and have happy lives, and we want to do this by advocating tailored nutrition as the foundation of holistic health.
      </MVCard>
      </Fade>
    </div>

    {/* Services Resources Overview */}
    <div className='flex flex-col justify-center items-center gap-10 w-full py-20 bg-secondary-200 text-primary'>
      <Fade direction='up'>
      <h1 className='text-4xl font-bold'>Services and Resources</h1>
      </Fade>
      <div className='grid grid-rows-2 grid-cols-4 gap-10'>
        {categories.map((item, index) =>
          <CategoryCard key={index} title={item.title} bg={item.bg} link={item.link}>{item.desc}</CategoryCard>
        )}
      </div>
    </div>

    {/* Testimonials */}
    <div id='testimonials' className='relative flex flex-col justify-center items-center gap-8 w-full h-screen'>
      <Fade direction='up' className='mt-12'>
      <h1 className=' text-secondary-200 text-4xl font-bold'>Testimonials</h1>
      </Fade>
      <div className='w-[75%] rounded-lg overflow-hidden'>
      <Marquee className='w-full h-full rounded-lg overflow-hidden' direction='right' autoFill pauseOnClick>
        {testimonials.map((item, index) =>
          <TestimonialCard key={index} name={item.name} image={item.image}>{item.review}</TestimonialCard>
        )}
      </Marquee>
      </div>

      {/* Sides design */}
      <Fade direction='left' className='absolute top-0 left-0 w-full h-full -z-10'>
      <img src={testimonial} className='absolute top-0 left-0 w-[12%] h-full -z-10 select-none' />
      </Fade>
      <Fade direction='right' className='absolute top-0 left-0 w-full h-full -z-10'>
      <img src={testimonial} className='absolute top-0 right-0 w-[12%] h-full -z-10 scale-x-[-1] select-none' />
      </Fade>
    </div>
    </>
  );
};

export default Home;