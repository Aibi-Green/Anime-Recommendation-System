...
def generate_macronutrients(i):
    macronutrients = fuzzy(i)
    return macronutrients
    
def recommend(dataframe,_input,parameters={'n_neighbors':5,'return_distance':False}):
    # create model
    model, scaler = knn_model(dataframe)
    
    # create fuzzy logic macronutrients
    print(_input)
    macronutrients = generate_macronutrients(_input)
    ...
    _input=np.array(list(macronutrients)).reshape(1,-1)
